...
class Pay
{
	...
	public function buyH5($arr) {
		...
		if($arr['channel']=='wx_pub'){
			...
			$unifiedOrder->setParameter("body",$arr['title']);//商品描述
			$unifiedOrder->setParameter("out_trade_no",$arr['order_no']);//商户订单号
			$unifiedOrder->setParameter("total_fee",$arr['amount']);//总金额
	...
	public function paySuccess($arr,$channel=1){	//1:微信  2:支付宝
		...
		$data = array('status'=>1,'channel'=>$channel);
		if(strstr($arr["out_trade_no"],'C')){
			$this->user=$user=Db::view("UserMoneyLog","user_id,money")->view("User","open_id,tel","User.user_id=UserMoneyLog.user_id")->where("UserMoneyLog.order_no='".$arr["out_trade_no"]."'")->find();
			$this->order_no=$arr['out_trade_no'];
			$this->ccc=$data;
			
			Db::transaction(function(){
				Db::name("UserMoneyLog")->where("order_no='".$this->order_no."'")->update($this->ccc);
				Db::name("User")->where("user_id=".$this->user['user_id'])->setInc("money",$this->user['money']);
			});
	...
	public function songMoney($arr){
		...
		if($arr['money']==10){
			$money=0;
		}else if($arr['money']==20){
			$money=0;
		}else if($arr['money']==30){
			$money=0;
		}else if($arr['money']==50){
			$money=0;
		}else if($arr['money']==100){
			$money=50;
		}else if($arr['money']==200){
			$money=50;
		}else{
			$money=0;
		}
		
		if(Db::name("User")->where("user_id=".$arr['user_id'])->setInc("song_money",100)){
			return 'OK';
		}else{
			return "NO";
		}
